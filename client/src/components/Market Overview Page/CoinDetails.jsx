...
export default function CoinDetails() {
  ...
  const addToWishListHandler = async () => {
    const data = {
      ...
      name: coinInfo.name,
      symbol: coinInfo.symbol,
      price: Number(coinInfo.priceUsd).toFixed(2),
      logo: logo,
      change: Number(coinInfo.changePercent24Hr).toFixed(2),
    };

    try {
      const result = await serverDataAPI.addToWishList(data);
      setWishlist(true);
      setInList(true);
    } catch (err) {
      setError(err.message);
            ...
            <div className="mt-4 sm:items-center sm:gap-4 sm:flex flex-col sm:flex-row">
              ...
              {!inList ? (
                <button
                  ...
                  className="flex items-center bg-yellow-100 text-yellow-800 text-sm font-medium me-2 px-2.5 py-0.5 rounded dark:bg-yellow-900 dark:text-yellow-300 mt-2 sm:mt-0 w-full sm:w-auto h-12 sm:h-auto"
                >
                  <svg
                    className="mr-2"
                    xmlns="http://www.w3.org/2000/svg"
